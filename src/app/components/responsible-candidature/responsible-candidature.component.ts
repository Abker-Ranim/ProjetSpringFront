...
@Component({
  selector: 'app-responsible-candidature',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './responsible-candidature.component.html',
  styleUrls: ['./responsible-candidature.component.css'],
})
export class ResponsibleCandidatureComponent implements OnInit {
  ...
  loadCandidatures(): void {
    this.roleRequestService.getTeamResponsibleRequestsByUser().subscribe({
  ...
  applyFilters(): void {
    ...
    filtered.sort(
      (a, b) => b.datePostulation.getTime() - a.datePostulation.getTime()
    );
  ...
  accepterCandidature(id: string): void {
    ...
    // TODO: Implement backend API call to accept candidature
    setTimeout(() => {
      const index = this.candidatures.findIndex((c) => c.id === id);
      if (index !== -1) {
        this.candidatures[index].statut = 'acceptee';
        console.log(
          `Email envoyé à ${this.candidatures[index].candidatEmail} pour l'informer que sa candidature a été acceptée.`
        );
      }
  ...
  refuserCandidature(id: string): void {
    ...
    // TODO: Implement backend API call to reject candidature
    setTimeout(() => {
      const index = this.candidatures.findIndex((c) => c.id === id);
      if (index !== -1) {
        this.candidatures[index].statut = 'refusee';
        console.log(
          `Email envoyé à ${this.candidatures[index].candidatEmail} pour l'informer que sa candidature a été refusée.`
        );
      }
